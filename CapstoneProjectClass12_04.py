...
def produce_csv(project_Array):
    with open('projects_file.csv', mode='w') as project_file:
        ...
        header = ["FloorSpaceID","Placed","Name","Length","Width","Position X","Position Y","Industry Type"]
        project_writer.writerow(header)
        for x in project_Array:
            ls = [x.project_FloorSpaceID,x.project_Placed,x.project_Name,x.project_Length,x.project_Width,x.project_Position[0],x.project_Position[1],x.project_Industry]
            try:
                project_writer.writerow(ls)
            except:
                continue

def produce_report(floorspace_Array, global_project_Array):
    unplacedProjs = []
    count = 0
    usedFloorSpace = []
...
p_dict = read_files("projects.csv").projects_dict
algo = run_Algorithm(p_dict)
print(algo.return_cluster())
